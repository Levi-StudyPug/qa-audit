...
def sanitize_filename(url):
    ...
    if filename.startswith('dev.studypug.com_'):
        filename = filename[len('dev.studypug.com_'):]
    if len(filename) > 100:
...
async def login_and_save_state(p, auth_url: str, role: str, locale:str):
    ...
    CREDENTIAL_ENV_MAP = {
        "ca": {
            "parent": ("SP_PARENT_USERNAME_DEV_CA", "SP_PARENT_PASSWORD_DEV_CA"),
            "student": ("SP_STUDENT_USERNAME_DEV_CA", "SP_STUDENT_PASSWORD_DEV_CA"),
        },
        "us": {
            "parent": ("SP_PARENT_USERNAME_DEV_US", "SP_PARENT_PASSWORD_DEV_US"),
            "student": ("SP_STUDENT_USERNAME_DEV_US", "SP_STUDENT_PASSWORD_DEV_US"),
        },
        "uk": {
            "parent": ("SP_PARENT_USERNAME_DEV_UK", "SP_PARENT_PASSWORD_DEV_UK"),
            "student": ("SP_STUDENT_USERNAME_DEV_UK", "SP_STUDENT_PASSWORD_DEV_UK"),
        },
        "ie": {
            "parent": ("SP_PARENT_USERNAME_DEV_IE", "SP_PARENT_PASSWORD_DEV_IE"),
            "student": ("SP_STUDENT_USERNAME_DEV_IE", "SP_STUDENT_PASSWORD_DEV_IE"),
        },
        "sg": {
            "parent": ("SP_PARENT_USERNAME_DEV_SG", "SP_PARENT_PASSWORD_DEV_SG"),
            "student": ("SP_STUDENT_USERNAME_DEV_SG", "SP_STUDENT_PASSWORD_DEV_SG"),
        },
        "au": {
            "parent": ("SP_PARENT_USERNAME_DEV_AU", "SP_PARENT_PASSWORD_DEV_AU"),
            "student": ("SP_STUDENT_USERNAME_DEV_AU", "SP_STUDENT_PASSWORD_DEV_AU"),
        },
        "nz": {
            "parent": ("SP_PARENT_USERNAME_DEV_NZ", "SP_PARENT_PASSWORD_DEV_NZ"),
            "student": ("SP_STUDENT_USERNAME_DEV_NZ", "SP_STUDENT_PASSWORD_DEV_NZ"),
        },
    }
...
async def take_screenshot(
    ...
    use_auth=False,
    auth_url="https://dev.studypug.com/signin/ca/?role=student",
    role: str | None = None,
...
async def main():
    ...
    json_path = Path(__file__).parent / "urls.json"
    ...
    # Process IE region
    ie_data = urls_data.get("ie", {})
    ie_guest = ie_data.get("guest_urls", [])
    ie_parent = ie_data.get("parent_urls", [])
    ie_student = ie_data.get("student_urls", [])
    
    # Process NZ region
    nz_data = urls_data.get("nz", {})
    nz_guest = nz_data.get("guest_urls", [])
    nz_parent = nz_data.get("parent_urls", [])
    nz_student = nz_data.get("student_urls", [])
    
    # Process SG region
    sg_data = urls_data.get("sg", {})
    sg_guest = sg_data.get("guest_urls", [])
    sg_parent = sg_data.get("parent_urls", [])
    sg_student = sg_data.get("student_urls", [])
    ...
    if ca_guest:
        ...
        await take_screenshots_batch(
            urls=ca_guest,
            output_dir="screenshots/ca/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
    ...
    if ca_parent:
        ...
        await take_screenshots_batch(
            urls=ca_parent,
            output_dir="screenshots/ca/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/ca/?role=parent",
            role="parent",
            locale="ca",
        )
        print()
    ...
    if ca_student:
        ...
        await take_screenshots_batch(
            urls=ca_student,
            output_dir="screenshots/ca/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/ca/?role=student",
    ...
    if us_guest:
        ...
        await take_screenshots_batch(
            urls=us_guest,
            output_dir="screenshots/us/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
    ...
    if us_parent:
        ...
        await take_screenshots_batch(
            urls=us_parent,
            output_dir="screenshots/us/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/us/?role=parent",
            role="parent",
            locale="us",
        )
        print()
    ...
    if us_student:
        ...
        await take_screenshots_batch(
            urls=us_student,
            output_dir="screenshots/us/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/us/?role=student",
            role="student",
            locale="us",
        )
        print()
    ...
    if uk_guest:
        ...
        await take_screenshots_batch(
            urls=uk_guest,
            output_dir="screenshots/uk/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
    ...
    if uk_parent:
        ...
        await take_screenshots_batch(
            urls=uk_parent,
            output_dir="screenshots/uk/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/uk/?role=parent",
            role="parent",
            locale="uk",
        )
        print()
    ...
    if uk_student:
        ...
        await take_screenshots_batch(
            urls=uk_student,
            output_dir="screenshots/uk/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/uk/?role=student",
            role="student",
            locale="uk",
        )
        print()
    ...
    if au_guest:
        ...
        await take_screenshots_batch(
            urls=au_guest,
            output_dir="screenshots/au/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
    ...
    if au_parent:
        ...
        await take_screenshots_batch(
            urls=au_parent,
            output_dir="screenshots/au/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/au/?role=parent",
            role="parent",
            locale="au",
        )
        print()
    ...
    if au_student:
        ...
        await take_screenshots_batch(
            urls=au_student,
            output_dir="screenshots/au/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/au/?role=student",
            role="student",
            locale="au",
        )
        print()
    ...
    # Take NZ guest screenshots
    if nz_guest:
        remove_all_auth_states()
        print(f"📸 NZ Guest Screenshots: {len(nz_guest)} URLs (no authentication)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=nz_guest,
            output_dir="screenshots/nz/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
     ...
     # Take NZ parent authenticated screenshots
    if nz_parent:
        print(f"🔐 NZ Parent Screenshots: {len(nz_parent)} URLs (requires parent login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=nz_parent,
            output_dir="screenshots/nz/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/nz/?role=parent",
            role="parent",
            locale="nz",
        )
        print()
    # Take NZ student authenticated screenshots
    if nz_student:
        remove_all_auth_states()
        print(f"🔐 NZ Student Screenshots: {len(nz_student)} URLs (requires student login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=nz_student,
            output_dir="screenshots/nz/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/nz/?role=student",
            role="student",
            locale="nz",
        )
        print()
     ...
     # Take IE guest screenshots
        
    if ie_guest:
        remove_all_auth_states()
        print(f"📸 IE Guest Screenshots: {len(ie_guest)} URLs (no authentication)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=ie_guest,
            output_dir="screenshots/ie/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
    ...
    if ie_parent:
        print(f"🔐 IE Parent Screenshots: {len(ie_parent)} URLs (requires parent login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=ie_parent,
            output_dir="screenshots/ie/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/ie/?role=parent",
            role="parent",
            locale="ie",
        )
        print()
    # Take IE student authenticated screenshots
    if ie_student:
        remove_all_auth_states()
        print(f"🔐 IE Student Screenshots: {len(ie_student)} URLs (requires student login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=ie_student,
            output_dir="screenshots/ie/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/ie/?role=student",
            role="student",
            locale="ie",
        )
        print()
    ...
    # Take SG guest screenshots
        
    if sg_guest:
        remove_all_auth_states()
        print(f"📸 SG Guest Screenshots: {len(sg_guest)} URLs (no authentication)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=sg_guest,
            output_dir="screenshots/sg/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
        print()    
     # Take SG parent authenticated screenshots
    if sg_parent:
        print(f"🔐 SG Parent Screenshots: {len(sg_parent)} URLs (requires parent login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=sg_parent,
            output_dir="screenshots/sg/parent",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/sg/?role=parent",
            role="parent",
            locale="sg",
        )
        print()
    
    # Take SG student authenticated screenshots
    if sg_student:
        remove_all_auth_states()
        print(f"🔐 SG Student Screenshots: {len(sg_student)} URLs (requires student login)")
        print("=" * 60)
        await take_screenshots_batch(
            urls=sg_student,
            output_dir="screenshots/sg/student",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=True,
            auth_url="https://dev.studypug.com/signin/sg/?role=student",
            role="student",
            locale="sg",
        )
        print()
    ...
    if shared_guest:
        ...
        await take_screenshots_batch(
            urls=shared_guest,
            output_dir="screenshots/shared/guest",
            viewport_width=1920,
            viewport_height=1080,
            use_auth=False
        )
